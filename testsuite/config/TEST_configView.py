...
from grid_control.config.config_entry import ConfigContainer, ConfigEntry, noDefault
...
def createHCV(cur = {}, old = None):
	...
	hcv._getSection = lambda *args, **kwargs: 'x'
	hcv._getSectionKey = lambda x: x
	return hcv

def showGet(hcv, option, default, persistent):
	try:
		formatValue = lambda x: str(x).replace("<class 'grid_control.utils.data_structures.Enum_3453_noDefault'>", '<no default>')
...
def doTest(cur, old, persistent):
	...
	showGet(hcv, 'key', noDefault, persistent)
	showGet(hcv, 'key', 'default1', persistent)
	showGet(hcv, 'key', noDefault, persistent)
	showGet(hcv, 'key', 'default2', persistent)
	hcv.write(sys.stdout, printTight = True)
