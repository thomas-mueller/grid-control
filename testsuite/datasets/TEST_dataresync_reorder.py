...
def testResync(ss, modstr, mode, nEvent = 3):
	config = create_config(configDict = {'mysection': {'events per job': nEvent, 'resync jobs': mode}})

	data_raw = ss2bl(ss)
	DataProvider.saveToFile('short_raw.dbs', data_raw)
	DataProvider.saveToFile('short_mod.dbs', modDS(data_raw, modstr))
	data_raw = DataProvider.loadFromFile('short_raw.dbs').getBlocks()
	data_mod = DataProvider.loadFromFile('short_mod.dbs').getBlocks()

	splitting_tmp = DataSplitter.createInstance('EventBoundarySplitter', config)
	splitting_tmp.splitDataset('short_raw.tar', data_raw)
	splitting_tmp.savePartitions('short_raw.tar', ss2sl(ss))

	splitting_raw = DataSplitter.loadPartitionsForScript('short_raw.tar', config)
	splitting_raw.resyncMapping('short_mod.tar', data_raw, data_mod)
	checkCoverage(splitting_raw, data_raw)

	splitting_mod = DataSplitter.loadPartitionsForScript('short_mod.tar')
	checkCoverage(splitting_mod, data_mod)

	def iterSplit(sp):
		for x in irange(sp.getMaxJobs()):
			yield sp.getSplitInfo(x)
	remove_files(['short_raw.tar', 'short_raw.dbs', 'short_mod.tar', 'short_mod.dbs'])
	return sl2ss(data_mod, iterSplit(splitting_mod))
