...
def createBroker(name, configDict, items = queues_base):
	random.seed(123)
	config = create_config(configDict={'broker': configDict})
	broker = Broker.createInstance(name, config, '', 'items', 'items', lambda: items)
	return broker
